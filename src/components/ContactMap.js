  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: '1.5rem' }}>
        {[
          ['Total Contacts', contacts.length, '#1A1A1A'],
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1rem' }}>
        {['all','distributor','retailer'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ background: filter === f ? '#1A1A1A' : '#FFF', color: filter === f ? '#FFF' : '#888', border: `0.5px solid ${filter === f ? '#1A1A1A' : '#E0DDD8'}`, borderRadius: 8, padding: '7px 16px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: filter === f ? 600 : 400, textTransform: 'capitalize' }}>
      ...
      {/* Map */}
      <div style={{ background: '#FFF', border: '0.5px solid #E8E4DF', borderRadius: 16, overflow: 'hidden', marginBottom: '1.5rem', position: 'relative' }}>
        {/* Note about addresses */}
        {withCoords === 0 && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, background: 'rgba(245,242,237,0.85)', backdropFilter: 'blur(4px)', flexDirection: 'column', gap: 8 }}>
            <div style={{ fontSize: 32, opacity: 0.3 }}>📍</div>
            <div style={{ fontSize: 14, color: '#AAA', textAlign: 'center', maxWidth: 300 }}>
        ...
        <svg viewBox={`0 0 ${PROJECT_W} ${PROJECT_H}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
          ...
          <rect width={PROJECT_W} height={PROJECT_H} fill="#F8F6F3" />
          <text x={PROJECT_W/2} y={PROJECT_H/2} textAnchor="middle" dominantBaseline="central" fill="#E0DDD8" fontSize={48} fontFamily="'Bebas Neue', sans-serif" letterSpacing="0.1em">UNITED STATES</text>
        ...
        <div style={{ position: 'absolute', bottom: 12, left: 12, display: 'flex', gap: 10 }}>
          {[['D', '#C9A84C', 'Distributor'], ['R', '#4CAF7D', 'Retailer']].map(([letter, color, label]) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(4px)', borderRadius: 6, padding: '4px 8px', border: '0.5px solid #E0DDD8' }}>
      ...
      {selected && (
        <div style={{ background: '#FFF', border: `0.5px solid ${TYPE_COLORS[selected.account_type] || '#E8E4DF'}`, borderRadius: 14, padding: '1.25rem', marginBottom: '1rem', position: 'relative' }}>
          <button onClick={() => setSelected(null)} style={{ position: 'absolute', top: 12, right: 12, background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#CCC', fontFamily: 'inherit' }}>×</button>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 13, color: '#888', marginTop: 2 }}>{selected.company}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
                {selected.email && <a href={`mailto:${selected.email}`} style={{ fontSize: 12, color: '#555', background: '#F8F6F3', padding: '4px 10px', borderRadius: 6, textDecoration: 'none' }}>📧 {selected.email}</a>}
                {selected.phone && <a href={`tel:${selected.phone}`} style={{ fontSize: 12, color: '#555', background: '#F8F6F3', padding: '4px 10px', borderRadius: 6, textDecoration: 'none' }}>📱 {selected.phone}</a>}
                {selected.phone && <a href={`https://wa.me/${selected.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#4CAF7D', background: '#F0FAF4', padding: '4px 10px', borderRadius: 6, textDecoration: 'none', border: '0.5px solid #C6EDD7' }}>💬 WhatsApp</a>}
              </div>
              {selected.address && <div style={{ fontSize: 12, color: '#AAA', marginTop: 6 }}>📍 {selected.address}</div>}
              {selected.store_type && <div style={{ fontSize: 12, color: '#AAA', marginTop: 4 }}>🏪 {selected.store_type} · {selected.location_count || 1} location{(selected.location_count || 1) > 1 ? 's' : ''}</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {/* Contact list */}
      <div style={{ fontSize: 12, color: '#AAA', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>Contact List</div>
      {loading && <div style={{ fontSize: 13, color: '#AAA' }}>Loading...</div>}
      {!loading && filtered.length === 0 && <div style={{ fontSize: 13, color: '#AAA' }}>No contacts yet.</div>}
      {filtered.map(c => (
        <div key={c.id} onClick={() => setSelected(c)}
          style={{ background: selected?.id === c.id ? '#FAFAFA' : '#FFF', border: `0.5px solid ${selected?.id === c.id ? (TYPE_COLORS[c.account_type] || '#E8E4DF') : '#E8E4DF'}`, borderRadius: 12, padding: '1rem', marginBottom: 8, cursor: 'pointer', transition: 'all 0.15s' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 500 }}>{c.name} — {c.company}</div>
              <div style={{ fontSize: 12, color: '#AAA', marginTop: 2 }}>{c.email} · {c.phone}</div>
              {c.address && <div style={{ fontSize: 11, color: '#CCC', marginTop: 2 }}>📍 {c.address}</div>}
            </div>
