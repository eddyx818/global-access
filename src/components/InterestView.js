    ...
    <div style={{ maxWidth: 620, margin: '0 auto', padding: isMobile ? 'max(8px, var(--ga-inset-top)) 1rem 2rem' : '1.5rem 1.5rem 3rem' }}>
      {!isMobile && (
        <>
          ...
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 38, letterSpacing: '0.04em', color: t.text, marginBottom: 4 }}>
            {staffPriceCheck ? 'Price check' : 'Request a Quote'}
          </div>
        </>
      )}
      ...
      {staffPriceCheck && (
        <div style={{ background: t.bgElevated, border: t.borderHairlineLight, borderRadius: 12, padding: '1.25rem', marginBottom: '1rem' }}>
          <label style={{ fontSize: 11, color: t.textFaint, display: 'block', marginBottom: 6, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Notes for your team</label>
          <textarea
            value={form.notes}
...
export function ThanksView({ onBack, onOpenSupport, chatLabel = 'Trade Desk', staffPriceCheck = false }) {
  const { t } = useTheme();
  ...
  return (
    <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 44, letterSpacing: '0.04em', color: t.text, marginBottom: 8 }}>
        {staffPriceCheck ? 'Sent to team!' : 'Quote Requested!'}
      </div>
      ...
      {onOpenSupport && (
        <button onClick={onOpenSupport} style={{ background: t.btnPrimaryBg, color: t.btnPrimaryText, border: 'none', borderRadius: 10, padding: '12px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 12 }}>
          Open {chatLabel}
        </button>
      )}
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: t.textFaint, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>← Back to catalog</button>
    </div>
  );
}
